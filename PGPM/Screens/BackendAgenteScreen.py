...
from Template.BackendAgenteView import BackendAgenteView
from Model.DAO.AgenteDAO import listaAgenti
from Model.DAO.AgenteDAO import deleteAgente
...
class BackendAgenteScreen:


	async def backendAgenteScreen(self,request):
		print('BACKEND Agente SCREEN')
		lista = listaAgenti()
		...
		session = returnSessione(session_id)
		wp = BackendAgenteView(lista,self.cancella,session)
		
		return wp

	def cancella(self,msg):
		print('cancella elemento')
		xid = list(msg.target.idAgente)
		deleteAgente(xid[0])
		msg.page.redirect = '/backend/gestioneAgente/'